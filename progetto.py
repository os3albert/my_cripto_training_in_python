...
'''
def simbolo_di_legendre(a, p):
    ls = a ** ((p - 1) / 2) % p
    return -1 if ls == p - 1 else ls  # ritorno 1 se a ha una radice quadrata modulo p, altrimenti -1

z = 0

# Implementare l’algoritmo indiano per il calcolo di potenze modulo m (pag. 47).
def algo_esponenzazione(base, esponente, modulo):
...
def algoShanksTonelli(n, p):
    '''
        n e p devono essere coprimi per trovare il la radice quadrata modulo p
    '''

    global z

    if gcd(n, p) != 1:
        print("n e p non sono coprimi");
        return -1

    # indica che la radice quadrata non è possibile
    if algo_esponenzazione(n, (p - 1) / 2, p) == (p - 1):
        print("la radice quadrata non è possibile")
        return -1
    ...
    q = 2
    while True:
        if algo_esponenzazione(q, (p - 1) / 2, p) == (p - 1):
            break
        q += 1

    x = algo_esponenzazione(n, (s + 1) / 2, p)
    b = algo_esponenzazione(n, s, p)
    g = algo_esponenzazione(q, s, p)
    ...
    while True:
        ...
        x = (x * algo_esponenzazione(g, int(2 ** (r - m - 1)), p)) % p
        g = algo_esponenzazione(g, int(2 ** (r - m)), p)
        b = (b * g) % p
...
n = 60  #103 #36  #9
p = 103 #641 #103 #103
x = algoShanksTonelli(n, p)

#output atteso

    #36 #49  #97  #100

print("la radice quadrata di ", n, " modulo ", p, " e': ", x, "\n")
